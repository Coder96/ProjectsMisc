...
use<C:/misc/scad/db_punchout.scad>;
...
if (1st_plate_bottom_hole == "keystone1" && 1st_plate == "none") {
	...
	else if (1st_plate_bottom_hole == "db09" && 1st_plate != "outlet" && 1st_plate != "toggle" && 1st_plate != "rocker") {
	translate([height_sizes[plate_size]/2 + 12,l_offset[plate_size],+3]) hole("db09_solid");
	}
}
...
if (2nd_plate == "keystone2" && 2nd_plate_bottom_hole == "none") {
	translate([height_sizes[plate_size]/2 + 14.3,l_offset[plate_size] - 2 + switch_offset,-3.9]) hole("keystone_solid");
	translate([height_sizes[plate_size]/2 + 14.3,l_offset[plate_size] - 23 + switch_offset,-3.9]) hole("keystone_solid");
	}
else if (2nd_plate == "keystone2" && 2nd_plate_bottom_hole != "outlet" && 2nd_plate_bottom_hole != "toggle" && 2nd_plate_bottom_hole != "rocker") {
	translate([height_sizes[plate_size]/2,l_offset[plate_size] - 2 + switch_offset,-3.9]) hole("keystone_solid");
	translate([height_sizes[plate_size]/2,l_offset[plate_size] - 23 + switch_offset,-3.9]) hole("keystone_solid");
	}
...
if (2nd_plate_bottom_hole == "keystone2" && 2nd_plate == "none") {
	...
	translate([height_sizes[plate_size]/2 + 28.5,l_offset[plate_size] - 2 + switch_offset,-3.9]) hole("keystone_solid");
	translate([height_sizes[plate_size]/2 + 28.5,l_offset[plate_size] - 23 + switch_offset,-3.9]) hole("keystone_solid");
	}
...
else if (5th_plate_bottom_hole == "db09" && 5th_plate != "outlet" && 5th_plate != "toggle" && 5th_plate != "rocker") {
	translate([height_sizes[plate_size]/2 + 28.5,l_offset[plate_size] - 11.5 + switch_offset * 4,-3.9]) hole("db09_solid");
	}
...
edgeheigth = 3;
	...
	hull(){
		cube([height_sizes[plate_size],solid_plate_width,heigth]);
		translate([0,0,3]) cylinder(h = heigth, r = cornerRad,center = true);
		translate([height_sizes[plate_size], solid_plate_width, 3]) cylinder(h = heigth, r = cornerRad,center = true);
		translate([height_sizes[plate_size], 0, 3]) cylinder(h = heigth, r = cornerRad,center = true);
		translate([0, solid_plate_width, 3]) cylinder(h = heigth, r = cornerRad,center = true);
	}
		translate([-4.3,-5,-(heigth-edgeheigth)]) rotate([0,0,0]) cube([10,edgewidth,6]); //Top Bevel 6.2
		translate([height_sizes[plate_size]-7,-5,-(heigth-edgeheigth)]) rotate([0,0,0]) cube([10,edgewidth,6]); //Bottom Bevel 6.25
		translate([height_sizes[plate_size]+10,-3,-(heigth-edgeheigth)]) rotate([0,0,90]) cube([10,height_sizes[plate_size]+20,6]); //Left Bevel (doesn't change) 6.1
		translate([height_sizes[plate_size]+10,rightbevel-3,-(heigth-edgeheigth)]) rotate([0,0,90]) cube([10,height_sizes[plate_size]+10,6]); //Right Bevel (scales right)
				}
			}
...
// Thinning Plate
module plate_inner() {
	scale([0.95,thinner_offset[plate_width],1.3]){
	translate([3,3,0]){
	difference() {
		translate([-2.5,-2.5,0]) cube([height_sizes[plate_size]+5,solid_plate_width+5,6]);
		translate([-8.3,-5,6.2]) rotate([0,45,0]) cube([6,edgewidth,6]); //Top Bevel
		translate([height_sizes[plate_size]-.2,-5,6.25]) rotate([0,45,0]) cube([6,edgewidth,6]); //Bottom Bevel
		translate([height_sizes[plate_size]+10,-8.4,6.1]) rotate([0,45,90]) cube([6,height_sizes[plate_size]+20,6]); //Left Bevel (doesn't change)
		translate([height_sizes[plate_size]+5,rightbevel+4,6]) rotate([0,45,90]) cube([6,height_sizes[plate_size]+10,6]); //Right Bevel (scales right)
				}
			}
		}
	}
...
module box_screws_support(){
	difference(){
	 difference(){
		translate([height_sizes[plate_size]/2 + 41.67125,0,3.5]) cylinder(r1=4, r2=6, h=3);
		translate([height_sizes[plate_size]/2 + 41.67125,0,-1]) cylinder(r=2, h=10, $fn=12);
	 }
	translate([height_sizes[plate_size]/2 + 41.67125,0,3.5]) cylinder(r1=2, r2=2.6, h=3.1);
  }
  difference(){
	 difference(){
	  translate([height_sizes[plate_size]/2 - 41.67125,0,3.5]) cylinder(r1=4, r2=6, h=3);
...
// DB09 Fits http://www.datapro.net/products/db9-serial-panel-mount-male-extension.html
	...
	if (hole_type == "vga_solid" || hole_type == "db09_solid") {
		
		translate([2,0,2.5]) rotate([0,180,0]) cutoutDB("DB09", "SLOT", "No", "Yes");
	}
...
//Keystone2 hole
//Hole for 2 Keystone Jacks
	if (hole_type == "keystone2") {
		
		translate([0,9.5,5]) cube([16.5,15,10], center = true);
//		translate([14.3,-2,-3.9]) hole("keystone_solid");
		translate([0,-11.5,5]) cube([16.5,15,10], center = true);
//		translate([14.3,-23,-3.9]) hole("keystone_solid2");
		
	}
//Keystone Solid2
	if (hole_type == "keystone_solid2") {
		rotate([0,0,90]) {
			difference(){
				translate([0,0,.1]) 
					cube([23,30.5,9.8]);
				...
				translate([0,0,.1]) 
					...
					translate([3,4,0])
						difference(){
							cube([17,22.5,10]);
			//				translate([-1,-0.001,3.501]) cube([17,2,6.5]);
							translate([-1,2.5,-3.40970]) rotate([45,0,0]) cube([19,2,6.5]);
		//					translate([-1,18.501,6.001]) cube([17,4,4]);
							translate([-1,20.5,0]) rotate([-45,0,0]) cube([19,2,6.5]);
						}
...
plate_width = 2;
1st_plate = "keystone1";
1st_plate_bottom_hole ="db09";
2nd_plate = "keystone2";
2nd_plate_bottom_hole ="keystone2";
...
union() {
		...
		translate([0,34.9,-1.68]) box_screws_support();
		translate([0,80.95,-1.68]) box_screws_support();
